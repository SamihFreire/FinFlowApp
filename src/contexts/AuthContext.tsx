...
type AuthContext = {
    isLoading: boolean
    session: null | UserLoginApiResponse,
    save: (data: UserLoginApiResponse) => void,
    remove: () => void
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
    const [session, setSession] = useState<null | UserLoginApiResponse>(null);
    const [isLoading, setIsLoading] = useState(true);

    // Função que guarda os dados da session
    function save(data: UserLoginApiResponse) {
        // Adicionando os dados da session no local storage do navegador
        // No local storage guardamos como texto os valores
        localStorage.setItem(`${LOCAL_STORAGE_KEY}:name`, JSON.stringify(data.name)); // Convertendo para string
    ...
    function loadUser() {
        const nameUser = localStorage.getItem(`${LOCAL_STORAGE_KEY}:name`);
        const token = localStorage.getItem(`${LOCAL_STORAGE_KEY}:token`);

        if(token && nameUser) {
            const name = JSON.parse(nameUser??"Visitante");
